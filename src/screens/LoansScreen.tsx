        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log('💼 Attempting to approve loan:', loan.id);
              
              // Use the original API function with correct endpoint
              const response = await approveLoanRequestApi(loan.group_id, loan.id, {
                dueDate: dueDate.toISOString(),
                interestRate: 10,
                paymentMethod: 'group_funds'
              });
              
              console.log('✅ Loan approval API response:', response.status, response.data);
              
              // Refresh the loan list to get updated status
              await loadGroupLoans();
              
              // Always show success if HTTP status is 2xx
              Alert.alert('Success', 'Loan request approved successfully');
              
            } catch (error: any) {
              console.error('❌ Loan approval error:', error);
              
              // Even if there's an error, refresh the list to check if it actually worked
              await loadGroupLoans();
              
              // Need to fetch the updated loan data to check actual status
              try {
                const groupLoansResponse = await getGroupLoanRequestsApi(selectedGroup);
                const updatedLoans = groupLoansResponse?.data?.data?.loans || [];
                const updatedLoan = updatedLoans.find(l => l.id === loan.id);
                
                if (updatedLoan && updatedLoan.status === 'approved') {
                  // If the loan is now approved despite the error, show success
                  console.log('✅ Loan status after error check: approved');
                  Alert.alert('Success', 'Loan request approved successfully');
                } else {
                ...
                // If we can't check the status, just show the original error
                const errorMessage = error.response?.data?.message || error.message || 'Failed to approve loan request';
                Alert.alert('Error', errorMessage);
              }
            }
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              console.log('💼 Attempting to reject loan:', loan.id);
              
              // Use the original API function with correct endpoint
              const response = await rejectLoanRequestApi(loan.group_id, loan.id);
              
              console.log('✅ Loan rejection API response:', response.status, response.data);
              ...
              // Even if there's an error, refresh the list to check actual status
              await loadGroupLoans();
              
              // Check if the loan was actually rejected
              try {
                const groupLoansResponse = await getGroupLoanRequestsApi(selectedGroup);
                const updatedLoans = groupLoansResponse?.data?.data?.loans || [];
                const updatedLoan = updatedLoans.find(l => l.id === loan.id);
                
                if (updatedLoan && updatedLoan.status === 'rejected') {
                  // If the loan is now rejected despite the error, show success
                  console.log('✅ Loan status after error check: rejected');
                  Alert.alert('Success', 'Loan request rejected successfully');
                } else {
                  // Otherwise show the error message
